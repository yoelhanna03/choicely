...
export async function POST(req: Request) {
  try {
    ...
    const [chatCompletion, summaryCompletion, scoreCompletion] =
      await Promise.all([
          ...
          messages: [
            {
              role: "system",
              content:
                "Tu es un assistant d'aide à la décision. Réponds de manière concise, encourage, sois sympa et prends tout en compte. Ne répète pas ce que l'utilisateur a dit.",
            },
            {
              role: "user",
          ...
          messages: [
            {
              role: "system",
              content:
                "Résume cette question en maximum 6 mots. Donne juste un titre court. Pas de phrases complètes (quelque chose de simple). Donne des titres général: pas de verbe conjugué, pas de pronom personnel. ",
            },
            {
              role: "user",
              content: situation,
          ...
          messages: [
            {
              role: "system",
              content:
                "Tu dois retourner UNIQUEMENT un nombre entre 0 et 100 (0=très mauvais, 50=moyen, 100=excellent). Ne retourne que le nombre, rien d'autre. Pas de texte, pas d'explication, pas de markdown. Juste le chiffre.Si par exemple ton score est de 50 retourne quelque chose comme 49, 51, 52, 48.",
            },
            {
              role: "user",
    ...
    console.log("[API /analyse] Summary RAW:", {
      raw: summaryRaw.slice(0, 100),
    });
    ...
    if (!summary || summary.length < 2) {
      ...
      summary =
        situation.split(" ").slice(0, 6).join(" ") || "Question sans titre";
    }
    ...
    let scoreText =
      scoreCompletion.choices[0]?.message?.content?.trim() ?? "50";
